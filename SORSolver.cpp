...
SORSolver::SORSolver(Real x0, Real y0, Real h, int nX, int nY,
                     Real w)
    : PDESolver(x0, y0, h, nX, nY), w(w) {}

Real SORSolver::iter(Real *sol, const PDE &pde) {
    ...
    for (int q = 0; q < nRegY; ++q) {
      ...
      for (int i = iInf; i <= iSup; ++i) {
        for (int j = jInf; j <= jSup; ++j) {
          int k = j - jInf;
          Real a = pde.next_value(x[i], y[j], mr[i][j], neum,
                                  b[k], t[k], l[k], r[k], h);
          sum += std::abs(w * (a - ms[i][j]));
